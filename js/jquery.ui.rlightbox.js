...
// its initial DOMstructure it transformed into the plugin's one. Therefore
	...
	global: {
		...
		getLightbox: function(){
			...
			if ( !$lb.root ){
				...
				// create the DOMstructure of the lightbox
		...
		removeSetElement: function( number ){
			...
			if( _total === 1 || _currentSet === "single" ){
				...
				// remove the instance from encapsulated DOMelement (jquery one)
				this.destroy();
			} else {
				this.destroy();
